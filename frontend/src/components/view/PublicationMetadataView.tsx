...
export const PublicationMetadataView: React.FC<
  PublicationMetadataViewProps
> = ({ publication }) => {
  if (!publication) return null;

  const totalPrepTime = publication.contents?.reduce(
    (acc, content) => acc + (content.total_prep_time || 0),
    0,
  );
    ...
    <div className="p-4 border border-gray-700 rounded-lg bg-[#1F1F1F]/80 my-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        <MetadataItem
          icon={<User size={16} />}
          label="Auteur"
          value={publication.author?.str_value}
        />
        <MetadataItem
          icon={<Hash size={16} />}
          label="Style"
          value={publication.style?.str_value}
        />
        <MetadataItem
          icon={<Hash size={16} />}
          label="Type"
          value={publication.type?.str_value}
        />
        {totalPrepTime && totalPrepTime > 0 ? (
          <MetadataItem
            icon={<Clock size={16} />}
            label="Temps total"
            value={`${totalPrepTime} min`}
          />
        ) : null}
