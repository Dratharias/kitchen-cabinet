...
async function createAndConnectIngredient(tx: Prisma.TransactionClient, payload: any) {
  assert(payload.product, "Product data is required", "createAndConnectIngredient");
  const productName = typeof payload.product === 'string' ? payload.product : payload.product.name;
  ...
  if (unitName) {
    const unit = await tx.unit.upsert({ where: { name: unitName }, create: { name: unitName }, update: {} });
    await tx.ingredient_unit.create({ data: { ingredient_id, unit_id: unit.unit_id } });
  }
...
async function createAndConnectSegment(tx: Prisma.TransactionClient, payload: any) {
  assert(payload.paragraph, "Segment paragraph is required", "createAndConnectSegment");
    ...
    for (const ptData of payload.segment_prep_time) {
      ...
      if (style?.str_value) {
        const upsertedStyle = await tx.category.upsert({
          where: { str_value_type: { str_value: style.str_value, type: style.type || 'PrepStyle' } },
          create: { str_value: style.str_value, type: style.type || 'PrepStyle' },
          update: {},
        });
      ...
      if (duration > 0) {
        const newPrepTime = await tx.prep_time.create({ data: { duration: Number(duration), style_id } });
        await tx.segment_prep_time.create({ data: { segment_id: newSegment.segment_id, prep_time_id: newPrepTime.prep_time_id } });
      }
    }
  }
...
async function handleAtomicContentUpdate(tx: Prisma.TransactionClient, id: string, payload: any) {
  ...
  if (payload.servings && typeof payload.servings === "object") {
    ...
    if (currentContent?.serving_id) {
      await tx.servings.update({ where: { serving_id: currentContent.serving_id }, data: payload.servings });
    } else {
      const newServing = await tx.servings.create({ data: { ...payload.servings, serving_id: uuidv4() } });
  ...
  if (payload.content_ingredients?.connect) {
    for (const ingData of payload.content_ingredients.connect) {
      ...
      await tx.content_ingredient.create({ data: { content_id: id, ingredient_id: newIngredientId } });
    }
  }

  if (payload.content_segments?.connect) {
    const lastPosition = (await tx.content_segment.findFirst({ where: { content_id: id }, orderBy: { position: 'desc' } }))?.position ?? 0;
    ...
    for (const segData of payload.content_segments.connect) {
      ...
      await tx.content_segment.create({ data: { content_id: id, segment_id: newSegmentId, position: currentPosition } });
    }
  }
  ...
  return tx.content.findUnique({
      ...
      include: { 
          servings: true, 
          content_segments: { include: { segment: true }, orderBy: { position: 'asc' } }, 
...
async function handleAtomicIngredientUpdate(tx: Prisma.TransactionClient, id: string, payload: any) {
  ...
  if (payload.multiply_factor !== undefined) data.multiply_factor = Number(payload.multiply_factor);

  if (payload.product && typeof payload.product === "string") {
    const product = await tx.product.upsert({ where: { name: payload.product }, create: { name: payload.product }, update: {} });
    data.product = { connect: { product_id: product.product_id } };
  }

  if (payload.unit && typeof payload.unit === "string") {
    const unit = await tx.unit.upsert({ where: { name: payload.unit }, create: { name: payload.unit }, update: {} });
    data.ingredient_units = { deleteMany: {}, create: { unit: { connect: { unit_id: unit.unit_id } } } };
  }
  
  if (payload.macro && typeof payload.macro === 'object') {
      const currentIngredient = await tx.ingredient.findUnique({ where: { ingredient_id: id }, select: { product: { select: { macro_id: true } } } });
      if (currentIngredient?.product?.macro_id) {
          await tx.macro.update({ where: { macro_id: currentIngredient.product.macro_id }, data: payload.macro });
      }
  }

  return tx.ingredient.update({ where: { ingredient_id: id }, data, include: { product: true, ingredient_units: { include: { unit: true } } } });
}
...
async function handleAtomicSegmentUpdate(tx: Prisma.TransactionClient, id: string, payload: any) {
    ...
    if (Array.isArray(segment_prep_time)) {
        ...
        if (segment_prep_time.length > 0) {
            ...
            await tx.segment_prep_time.createMany({ data: newPrepTimeIds.map(prep_time_id => ({ segment_id: id, prep_time_id })) });
        }
    }
...
async function handleAtomicProductUpdate(tx: Prisma.TransactionClient, id: string, payload: any) {
  return tx.product.update({ where: { product_id: id }, data: payload });
}
async function handleAtomicMacroUpdate(tx: Prisma.TransactionClient, id: string, payload: any) {
  return tx.macro.update({ where: { macro_id: id }, data: payload });
}
async function handleAtomicUnitUpdate(tx: Prisma.TransactionClient, id: string, payload: any) {
  return tx.unit.update({ where: { unit_id: id }, data: payload });
}
async function handleAtomicCategoryUpdate(tx: Prisma.TransactionClient, id: string, payload: any) {
  return tx.category.update({ where: { category_id: id }, data: payload });
}
async function handleAtomicPrepTimeUpdate(tx: Prisma.TransactionClient, id: string, payload: any) {
  return tx.prep_time.update({ where: { prep_time_id: id }, data: payload });
}
async function handleAtomicReviewUpdate(tx: Prisma.TransactionClient, id: string, payload: any) {
  return tx.review.update({ where: { review_id: id }, data: payload });
}
async function handleAtomicServingsUpdate(tx: Prisma.TransactionClient, id: string, payload: any) {
...
export class OrchestratorController {
  public async processRequest(req: OrchestratorRequest): Promise<OrchestratorResponse> {
    ...
    try {
      const results = await prisma.$transaction(async (tx: any) => {
        ...
        if (action === "create") {
          for (const key in payload) {
            ...
            assert(data, "Missing payload for create", "processRequest:create", `payload.${key}`);
            if (data.title && Array.isArray(data.contents)) {
              out[key] = await processor.create({ publication_id: key, ...data });
            } else {
               throw new OrchestratorError(`'${key}' is not a valid structure for a 'create' action.`, "processRequest:create");
            }
          }
        } else {
          for (const resourceType of Object.keys(payload || {})) {
            const entities = (payload as any)[resourceType];
            assert(entities && typeof entities === 'object', `Invalid payload for '${resourceType}'`, `processRequest:${action}`);
            for (const id in entities) {
              const data = entities[id];
              if (action === "update") {
                const handlerMap: Record<string, Function> = {
                  publications: handleAtomicPublicationUpdate,
                ...
                if (!handlerMap[resourceType]) throw new OrchestratorError(`Update not supported for '${resourceType}'`, "processRequest:update");
                ...
                await (tx[modelName] as any).delete({ where: { [idFieldName]: id } });
      ...
      return { success: false, error: error instanceof OrchestratorError ? `[${error.context}] ${error.message}` : "Internal server error" };
    }
  }
}