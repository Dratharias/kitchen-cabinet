...
class OrchestratorError extends Error {
  ...
  path?: string;
  payload?: unknown;
  constructor(message: string, context: string, path?: string, payload?: unknown) {
    super(message);
...
function logError(context: string, error: any, extra?: Record<string, unknown>) {
...
export class OrchestratorController {
  ...
  public async processRequest(req: OrchestratorRequest): Promise<OrchestratorResponse> {
      ...
      try {
        const results = await prisma.$transaction(async (tx: any) => {
          ...
          for (const key of Object.keys(payload || {})) {
            ...
            assert(publication, "Missing publication payload", "processRequest", `payload.${key}`, publication);
        ...
        return {
          success: false,
          error: DEV_MODE ? `[transaction] ${error?.message || "Internal server error"}` : "Internal server error",
        };
      }
    }
    ...
    if (action === "readAll") {
      try {
        ...
        return { success: true, results: { categories, products, units } as any };
      } catch (error: any) {
  ...
  private async createPublication(pub: any, tx: PrismaClient) {
    ...
    const publication_id = await this.safeId(tx, "publication", "publication_id", pub.publication_id);
    ...
    if (Array.isArray(pub.contents)) {
      for (let i = 0; i < pub.contents.length; i++) {
        await this.processContent(pub.contents[i], created.publication_id, tx, `contents[${i}]`);
      }
    }
  ...
  private async updatePublication(pub: any, tx: PrismaClient) {
    ...
    await tx.publication_tag.deleteMany({ where: { publication_id: updated.publication_id } });
    if (Array.isArray(pub.tags)) {
      for (let i = 0; i < pub.tags.length; i++) {
        const tagId = await this.processCategory(pub.tags[i], tx, "Tag", `tags[${i}]`);
        if (!tagId) continue;
    ...
    await tx.content.deleteMany({ where: { publication_id: updated.publication_id } });
    if (Array.isArray(pub.contents)) {
      for (let i = 0; i < pub.contents.length; i++) {
        await this.processContent(pub.contents[i], updated.publication_id, tx, `contents[${i}]`);
      }
    }
  ...
  /* -------------------- content (flat) -------------------- */
  private async processContent(content: any, publicationId: string, tx: PrismaClient, path = "content") {
    const ctx = "processContent";
    try {
      assert(content && typeof content === "object", "Invalid content payload", ctx, path, content);

      const content_id = await this.safeId(tx, "content", "content_id", content.content_id);
      ...
      const created = await tx.content.create({
        data: {
          ...
          servings:
            typeof content.servings === "number" || content.servings === null ? content.servings : null,
      ...
      if (Array.isArray(content.content_segments)) {
        for (let i = 0; i < content.content_segments.length; i++) {
          ...
          if (!seg) continue;
          const segment_id = await this.processSegment(seg, tx, `${path}.content_segments[${i}].segment`);
      ...
      if (Array.isArray(content.content_ingredients)) {
        for (let i = 0; i < content.content_ingredients.length; i++) {
          const ing_id = await this.processIngredient(content.content_ingredients[i], tx, `${path}.content_ingredients[${i}]`);
      ...
      if (Array.isArray(content.content_prep_times)) {
        for (let i = 0; i < content.content_prep_times.length; i++) {
          const pt_id = await this.processPrepTime(content.content_prep_times[i], tx, `${path}.content_prep_times[${i}]`);
  ...
  private async processCategory(cat: any, tx: PrismaClient, fallbackType: string, path = "category"): Promise<string | null> {
    if (!cat) return null;
    ...
    const existing = await tx.category.findUnique({ where: { str_value_type: { str_value, type } } });
    if (existing) return existing.category_id;

    const category_id = uuidv4();
    const created = await tx.category.create({ data: { category_id, str_value, type } });
    return created.category_id;
  }

  private async processProduct(prod: any, tx: PrismaClient, path = "product"): Promise<string> {
    assert(prod, "Product payload missing", "processProduct", path, prod);
    const name = (prod.name ?? "").toString().trim();
    assert(name, "Product.name is required", "processProduct", `${path}.name`, prod);

    const existing = await tx.product.findUnique({ where: { name } });
    if (existing) return existing.product_id;

    const product_id = uuidv4();
    const created = await tx.product.create({ data: { product_id, name, en_name: name } });
    return created.product_id;
  }

  private async processUnit(unit: any, tx: PrismaClient, path = "unit"): Promise<string> {
  ...
  private async processIngredient(ing: any, tx: PrismaClient, path = "ingredient"): Promise<string> {
    assert(ing, "Ingredient payload missing", "processIngredient", path, ing);
    const product_id = await this.processProduct(ing.product, tx, `${path}.product`);
    ...
    if (Array.isArray(ing.ingredient_units)) {
      for (let i = 0; i < ing.ingredient_units.length; i++) {
        const unitId = await this.processUnit(ing.ingredient_units[i]?.unit, tx, `${path}.ingredient_units[${i}].unit`);
  ...
  private async processSegment(seg: any, tx: PrismaClient, path = "segment"): Promise<string> {
    assert(seg, "Segment payload missing", "processSegment", path, seg);
    const paragraph = (seg.paragraph ?? "").toString().trim();
    assert(paragraph, "Segment.paragraph is required", "processSegment", `${path}.paragraph`, seg);
    ...
    if (existing) {
      ...
      if (typeof seg.title === "string" && seg.title !== existing.title) {
        await tx.segment.update({ where: { segment_id: existing.segment_id }, data: { title: seg.title } });
      }
  ...
  private async processPrepTime(pt: any, tx: PrismaClient, path = "prep_time"): Promise<string> {
    assert(pt, "PrepTime payload missing", "processPrepTime", path, pt);
    const duration = Number(pt.duration);
    assert(Number.isFinite(duration), "PrepTime.duration is required (number)", "processPrepTime", `${path}.duration`, pt);

    let style_id: string | null = null;
    if (pt.style) {
      style_id = await this.processCategory(pt.style, tx, "Cook", `${path}.style`);
    }
