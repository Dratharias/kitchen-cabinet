 ...
 */
async function handleAtomicIngredientCreate(
  ...
  assert(
    ...
    "Product name is required for new ingredient",
    "handleAtomicIngredientCreate",
  );
 ...
 */
async function handleAtomicSegmentCreate(
  tx: Prisma.TransactionClient,
  contentId: string,
  payload: any,
) {
...
async function handleAtomicIngredientUpdate(
  ...
  if (
    payload.ingredient_units &&
    Array.isArray(payload.ingredient_units) &&
    payload.ingredient_units[0]?.unit?.name
  ) {
    const unitName = payload.ingredient_units[0].unit.name;
    const unit = await tx.unit.upsert({
      where: { name: unitName },
          ...
          for (const key of Object.keys(payload || {})) {
            ...
            if (action === "create") {
              ...
              if (data.title && Array.isArray(data.contents)) {
                ...
                if (data.product && Array.isArray(data.ingredient_units)) {
                  assert(
                    ...
                    `Parent content with ID ${parentId} not found for ingredient creation.`,
                    "processRequest:create",
                  );
                  newEntity = await handleAtomicIngredientCreate(
                    tx,
                    parentId,
                    data,
                  );
                ...
                // Check if it's a segment being added to a content
                  ...
                  assert(
                    ...
                    `Parent content with ID ${parentId} not found for segment creation.`,
                    "processRequest:create",
                  );
                  newEntity = await handleAtomicSegmentCreate(
                    tx,
                    parentId,
                    data,
                  );
                  ...
                  throw new OrchestratorError(
                    `Unknown payload structure for 'create' action.`,
                    "processRequest:create",
                    `payload.${key}`,
                  );
                }
              ...
              // Case 2: Atomic update on a single entity
                ...
                if (
                  await tx.ingredient.findUnique({
                    where: { ingredient_id: entityId },
                  })
                ) {
                  updatedEntity = await handleAtomicIngredientUpdate(
                    tx,
                    entityId,
                    data,
                  );
                } else if (
                  await tx.segment.findUnique({
                    where: { segment_id: entityId },
                  })
                ) {
                  updatedEntity = await handleAtomicSegmentUpdate(
                    tx,
                    entityId,
                    data,
                  );
                  ...
                  await tx.content.findUnique({
                    where: { content_id: entityId },
                  })
                ) {
                  updatedEntity = await handleAtomicContentUpdate(
                    tx,
                    entityId,
                    data,
                  );
                  ...
                  updatedEntity = await handleAtomicPublicationUpdate(
                    tx,
                    entityId,
                    data,
                  );
                  ...
                  throw new OrchestratorError(
                    `Entity with ID ${entityId} not found for atomic update.`,
              ...
              if (
                ...
                await tx.content.findUnique({ where: { content_id: entityId } })
              ) {
                ...
                await tx.segment.findUnique({ where: { segment_id: entityId } })
              ) {
                await tx.segment.delete({ where: { segment_id: entityId } });
                deleted = true;
              } else {
                throw new OrchestratorError(
                  `Entity with ID ${entityId} not found for atomic delete.`,
