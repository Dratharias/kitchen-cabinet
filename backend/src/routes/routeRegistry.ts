...
export class RouteRegistry {
  ...
  registerCrud<T, C, U>(
    ...
    if (methods.includes("create")) {
      this.fastify.post(
        path,
        getWriteHandler(async (req: any, reply) => {
    ...
    if (methods.includes("findById")) {
      this.fastify.get(
        `${path}/:id`,
        getReadHandler(async (req: any, reply) => {
  ...
  registerOrchestratorRoute(
    ...
    const handler =
      authRequired && this.authHandlers.authGuard
    ...
    this.fastify[method.toLowerCase() as "post" | "get"](
      path,
  ...
  registerCustomRoute(
    ...
    this.fastify[method.toLowerCase() as "get" | "post" | "put" | "delete"](
      path,
