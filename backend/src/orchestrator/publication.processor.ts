...
export class PublicationProcessor {
  ...
  async create(pub: any) {
    ...
    const type_id = await this.atomProcessor.processCategory(
      pub.type,
      "Type",
      `${ctx}.type`,
    );
    const style_id = await this.atomProcessor.processCategory(
      pub.style,
      "Style",
      `${ctx}.style`,
    );
    const author_id = await this.atomProcessor.processCategory(
      pub.author,
      "Author",
      `${ctx}.author`,
    );
  ...
  async update(pub: any) {
    ...
    await this.tx.publication_tag.deleteMany({
    ...
    if (contentIds.length > 0) {
      await this.tx.content_ingredient.deleteMany({
        where: { content_id: { in: contentIds } },
      });
      await this.tx.content_segment.deleteMany({
        where: { content_id: { in: contentIds } },
      });
      await this.tx.content_prep_time.deleteMany({
        where: { content_id: { in: contentIds } },
      });
      await this.tx.content.deleteMany({
        where: { publication_id: pub.publication_id },
      });
    }
  ...
  private async processRelations(
    ...
    const subRecipeContents = contents.filter(
      (c: any) => c.is_ingredient === true,
    );
    const mainContents = contents.filter((c: any) => !c.is_ingredient);

    console.log(
      `[PublicationProcessor] Processing publication '${pub.title}' (${publication_id})`,
    );
    console.log(
      `[PublicationProcessor] Found ${subRecipeContents.length} sub-recipes, ${mainContents.length} main contents`,
    );
    ...
    for (const [index, recipeContent] of subRecipeContents.entries()) {
      assert(
        recipeContent.subtitle,
        "Content with is_ingredient=true must have a subtitle",
        "processRelations",
      );
      ...
      console.log(
        `[PublicationProcessor] Creating sub-recipe ${index + 1}/${subRecipeContents.length}: '${subRecipeTitle}' (${subRecipeId})`,
      );
    ...
    console.log(
      `[PublicationProcessor] Created ${subRecipePublications.length} sub-recipe publications`,
    );
    ...
    for (const subRecipe of subRecipePublications) {
      ...
      for (const content of mainContents) {
        if (Array.isArray(content.content_ingredients)) {
          for (const ingredient of content.content_ingredients) {
            ...
            console.log(
              `[PublicationProcessor] Score '${productName}' vs '${subRecipe.title}' = ${score.toFixed(3)}`,
            );
      ...
      if (bestMatch) {
        ...
        if (forced) {
          console.log(
            `[PublicationProcessor] Sub-recipe '${subRecipe.title}' forced match with product '${bestMatch.ingredient.product?.name}' (score: ${bestMatch.score.toFixed(3)}, below threshold)`,
          );
        } else {
          console.log(
            `[PublicationProcessor] Sub-recipe '${subRecipe.title}' matched with product '${bestMatch.ingredient.product?.name}' (score: ${bestMatch.score.toFixed(3)}, above threshold)`,
          );
        }
      } else {
        matches.push({
    ...
    if (Array.isArray(pub.tags)) {
      for (const [i, tag] of pub.tags.entries()) {
        const tagId = await this.atomProcessor.processCategory(
          tag,
          "Tag",
          `tags[${i}]`,
        );
        if (tagId) {
          await this.tx.publication_tag.create({
            data: { publication_id, category_id: tagId },
          });
        }
      }
    }
    ...
    console.log(
      `[PublicationProcessor] Completed processing for publication '${pub.title}'`,
    );
  }
}
