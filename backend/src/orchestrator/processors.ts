  ...
  async processProduct(prod: any, path: string): Promise<string> {
    ...
    return (
      await this.tx.product.create({
        data: { product_id: uuidv4(), name, macro_id },
      })
    ).product_id;
  }
...
class ContentProcessor {
  ...
  private async processContentSegment(
    ...
    const segment_id = await this.atomProcessor.processSegment(
      wrap.segment,
      `${path}.content_segments[${index}].segment`,
    );
  ...
  private async processContentIngredient(
    ...
    const ingredient_id = await this.atomProcessor.processIngredient(
      ing,
      `${path}.content_ingredients[${index}]`,
    );
    await this.tx.content_ingredient.create({
      data: { content_id, ingredient_id },
    });
  }
